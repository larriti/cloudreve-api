...
pub use auth::{CaptchaInfo, LoginResponse, TokenInfo, V3LoginResponse, V4LoginResponse};
...
pub struct CloudreveAPI {
    inner: UnifiedClient,
    base_url: String,
    /// Store V4 2FA session ID for finish_2fa_login
    v4_session_id: Option<String>,
}
...
impl CloudreveAPI {
    ...
    ///
    /// # Arguments
    /// * `path` - Full path where the file will be uploaded (e.g., "/folder/file.txt")
    /// * `content` - File content as bytes
    /// * `policy_id` - Optional storage policy ID (uses default if None)
    ...
    pub async fn upload_file(
        ...
        self.inner
            .upload_file(path, content, policy_id, overwrite)
            .await
    }
}
...
impl Clone for CloudreveAPI {
    fn clone(&self) -> Self {
        Self {
            ...
            v4_session_id: self.v4_session_id.clone(),
        }
    }
}
