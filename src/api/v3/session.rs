...
impl ApiV3Client {
    ...
    pub async fn login(&mut self, request: &LoginRequest<'_>) -> Result<User, Error> {
        ...
        let mut new_session_cookie: Option<String> = None;
                ...
                && cookie_str.contains("cloudreve-session=")
            {
                for part in cookie_str.split(';') {
                    ...
                    if part.starts_with("cloudreve-session=") {
                        ...
                        new_session_cookie = Some(session_value.to_string());
                        debug!(
                            "Extracted V3 session cookie from response: {}...",
        ...
        // Only save session cookie if login was successful
        match api_response.data {
            Some(_) => {
                if let Some(ref cookie) = new_session_cookie {
                    self.session_cookie = Some(cookie.clone());
                    debug!(
                        "V3 session_cookie after successful login: {:?}",
                        self.session_cookie
                    );
                }
            }
            None => {
                debug!("Login failed, not updating session cookie");
            }
        }
        ...
        match api_response.data {
            Some(user) => Ok(user),
            None => {
                // Code 203 indicates two-factor authentication is required
                // V3 sets session cookie even when 2FA is required
                if api_response.code == 203 {
                    // Save session cookie for 2FA request
                    if let Some(ref cookie) = new_session_cookie {
                        self.session_cookie = Some(cookie.clone());
                        debug!("V3 session_cookie saved for 2FA: {:?}", self.session_cookie);
                    }
                    Err(Error::TwoFactorRequired(String::new()))
                } else {
                    Err(Error::Api {
