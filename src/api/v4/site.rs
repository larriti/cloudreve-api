...
impl ApiV4Client {
    ...
    pub async fn get_captcha(&self) -> Result<CaptchaResponse, Error> {
        let response: crate::ApiResponse<CaptchaResponse> = self.get("/site/captcha").await?;
        match response.data {
    ...
    pub async fn check_captcha_requirement(&self) -> Result<bool, Error> {
        let config = self.get_site_config(SiteConfigSection::Login).await?;
        Ok(config.login_captcha.unwrap_or(false))
    }
}
