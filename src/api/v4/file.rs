...
impl ApiV4Client {
    ...
    ///
    /// Creates permanent direct links that can be used to access file content directly.
    /// Only file owners or administrators can create direct links.
    ///
    /// # Arguments
    ...
    pub async fn create_direct_link(
        ...
        let converted_request = CreateDirectLinkRequest { uris: uris_refs };

        let response: ApiResponse<Vec<DirectLinkItem>> =
            self.put("/file/source", &converted_request).await?;
        match response.data {
            Some(data) => Ok(data),
            None => Err(Error::InvalidResponse(format!(
                "API returned no data for create_direct_link request: {:?}",
                response
            ))),
        }
    }
    ...
    ///
    /// Only file owners can delete direct links.
    ///
    /// # Arguments
    /// * `id` - ID of the direct link to delete
    pub async fn delete_direct_link(&self, id: &str) -> Result<(), Error> {
        let response: ApiResponse<()> = self.delete(&format!("/file/source{}", id)).await?;
        match response.code {
            0 => Ok(()),
            code => Err(Error::Api {
                code,
